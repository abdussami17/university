                ...
                <div class="main-section">
                    <div class="card mt-5">
                        <div class="card-header">
                            <h2>{{ trans('general.Upload_Student_Card') }}</h2>
                        </div>
                        <div class="card-body">                
                            <form action="{{ route('student.verify.card') }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                <input type="hidden" name="user_id" value="{{ $user->id }}">
                        
                                <label for="student_card">{{ trans('general.Upload_Student_Card') }}:</label>
                                <input type="file" name="student_card" id="student_card" accept="image/*" required class="form-control" onchange="previewImage(event)"><br>
                        
                                <!-- Image Preview -->
                                <div id="image-preview" style="display: none; margin-top: 10px;">
                                    <img id="preview-img" src="" alt="Selected Image" style="width: 300px; height: 300px; border-radius: 10px;">
                                </div>
                        
                                <button type="submit" class="btn btn-primary mt-3">{{ trans('general.Verify') }}</button>
                            </form>
                        </div>
