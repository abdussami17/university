@extends('user.layout')
@section('title',trans('general.Affiliate_Programs_Detail'))
@section('content')
<div class="mt-60">
    <div class="card">
        <div class="card-body">
    ...
    <h4>{{ trans('general.Affiliate_Programs_Detail') }}</h4>
...
<table class="table table-bordered" id="myTableunique">
    ...
    <tbody>
        @php
            $userId = auth()->id();
            $workshops = App\Models\Enroll::where('module_type', 'affiliate-program')->where('user_id', $userId)->get();
        @endphp

        @foreach ($workshops as $index => $enroll)
           @php
               $workshopname = App\Models\AffliatePrograms::where('id', $enroll->module_id)->first();
           @endphp
            <tr>
                <td>{{ $index + 1 }}</td>
