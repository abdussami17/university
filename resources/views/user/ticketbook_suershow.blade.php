@extends('user.layout')
@section('title',trans('general.Tickets_Booked_Detail'))
@section('content')
<div class="mt-60">
    <div class="card">
        <div class="card-body">
    ...
    <h4>{{ trans('general.Tickets_Booked_Detail') }}</h4>
                        @php
                            $userIds = auth()->id();
                            $workshops=App\Models\Enroll::where('module_type','course')->where('user_id',$userIds)->get();
                        @endphp
...
<table class="table table-bordered" id="myTableunique">
    ...
    <tbody>
        @php
            $userId = auth()->id();
            $workshops = App\Models\Enroll::where('module_type', 'travel')->where('user_id', $userId)->get();
        @endphp

        @foreach ($workshops as $index => $enroll)
           @php
               $workshopname = App\Models\TravelMobility::where('id', $enroll->module_id)->first();
           @endphp
            <tr>
                <td>{{ $index + 1 }}</td>
