...
<div class="container-fluid">
    ...
    <div class="py-3 d-flex align-items-sm-center flex-sm-row flex-column">
        <div class="flex-grow-1">
            <h4 class="fs-18 fw-semibold m-0">{{ trans('general.Affiliate_Programs') }}</h4>
        </div>
                    ...
                    <div class="modal-dialog modal-dialog-scrollable  modal-dialog-centered">
                      <div class="modal-content">
                            <div class="modal-header">
                                <h1 class="modal-title fs-5" id="exampleModalScrollableTitle">{{ trans('general.Add_Affiliate_Program') }}</h1>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div class="modal-body">
                              <div class="row ">
                                <form enctype="multipart/form-data" class="d-flex flex-wrap justify-content-between" action="{{route('admin.add-affiliate')}}" method="post">
                                @csrf 
    ...
    @foreach ($data as $index => $item)
        <tr>
            ...
            <td class="text-end d-flex">
                <a href="{{ route('admin.edit-affiliate', $item->id) }}" class="btn btn-sm bg-primary-subtle me-1">
...
<tr>
  <td></td>
    ...
    <tr>
        <td colspan="5" class="text-center text-danger">{{ trans('general.No_Affiliate_Programs') }}</td>
    </tr>
@endif
...
<div class="modal fade" id="deleteModal" tabindex="-1" aria-labelledby="deleteModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h1 class="modal-title fs-5" id="deleteModalLabel">{{ trans('general.Confirm_Delete', ['attribute' => trans('general.Affiliate_Program')]) }}</h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <p>{{ trans('general.Confirm_Delete_Message', ['attribute' => trans('general.Affiliate_Program')]) }}</p>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-light" data-bs-dismiss="modal">{{ trans('general.Close') }}</button>
...
document.addEventListener("DOMContentLoaded", function () {
    document.querySelectorAll(".delete-btn").forEach(button => {
        button.addEventListener("click", function () {
            ...
            let actionUrl = "{{ route('admin.affiliate-delete', ':id') }}".replace(':id', userId);
