...
<div class="container-fluid">
    ...
    <div class="py-3 d-flex align-items-sm-center flex-sm-row flex-column">
        <div class="flex-grow-1">
            <h4 class="fs-18 fw-semibold m-0">{{ trans('general.Travel_Mobility') }}</h4>
        </div>
                    ...
                    <div class="modal-dialog modal-dialog-scrollable  modal-dialog-centered">
                      <div class="modal-content">
                            <div class="modal-header">
                                <h1 class="modal-title fs-5" id="exampleModalScrollableTitle">{{ trans('general.Add_Travel_Mobility') }}</h1>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div class="modal-body">
                              <div class="row ">
                                <form enctype="multipart/form-data" class="d-flex flex-wrap justify-content-between" action="{{route('admin.add-travel-mobility')}}" method="post">
                                @csrf 
                                ...
                                <div class="mb-3">
                                 <label for="email" class="form-label">{{ trans('general.Price') }}</label>
                                ...
                                <div class="mb-3">
                                 <label for="texteditor3" class="form-label">{{ trans('general.Description') }}</label>
                                    ...
                                    <div class="col-lg-5 col-12">
                                        ...
                                        <div class="mb-3">
                                            <label for="" class="form-label">{{ trans('genral.Select_Category') }}</label>
                                               <select required class="form-select" name="category_id">
                                               <option value="1">{{ trans('general.Bus_Ticket') }}</option>
                                               <option value="2">{{ trans('general.Train_Ticket') }}</option>
                                               <option value="3">{{ trans('general.Plane_Ticket') }}</option>
                                               <option value="4">{{ trans('general.Concert_Ticket') }}</option>
                                           </select>
                                        </div>
                                        <!-- <div class="mb-3">
                    ...
                    <table class="table datatable" id="datatable_1">
                            ...
                            @foreach ($data as $index => $item)
                                <tr>
                                    ...
                                    <a href="{{ route('admin.edit-travel', $item->id) }}" class="btn btn-sm bg-primary-subtle me-1">
                            ...
                            @endforeach
                                ...
                                <div class="modal fade" id="deleteModal" tabindex="-1" aria-labelledby="deleteModalLabel" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h1 class="modal-title fs-5" id="deleteModalLabel">{{ trans('general.Confirm_Delete', ['attribute' => trans('general.Travel_Mobility')]) }}</h1>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                            </div>
                                            <div class="modal-body">
                                                <p>{{ trans('general.Confirm_Delete_Message', ['attribute' => trans('general.Travel_Mobility')]) }}</p>
                                            </div>
                                            <div class="modal-footer">
...
document.addEventListener("DOMContentLoaded", function () {
    document.querySelectorAll(".delete-btn").forEach(button => {
        button.addEventListener("click", function () {
            ...
            let actionUrl = "{{ route('admin.travel-mobility-delete', ':id') }}".replace(':id', userId);
