...
class LoginController extends Controller
{
    ...
    public function authenticate(Request $req)
    {
        ...
        if (!$user) {
            return redirect()->route('account.login')->with('error', __('auth.email_not_exist'));
        }
    

        if (!Hash::check($req->password, $user->password)) {
            return redirect()->route('account.login')->with('error', __('auth.incorrect_password'));
        }
        ...
        if (Auth::attempt(['email' => $req->email, 'password' => $req->password])) {
            if (Auth::user()->role != 'user') {
                Auth::logout();
                return redirect()->route('account.login')->with('error', __('auth.not_authorize_page'));
            }
            ...
            // Check if module_id and module_type are stored in session
            if (session()->has('module_type') && session()->has('module_id')) {
                $moduleType = session('module_type');
                $moduleId = session('module_id');

                session()->forget(['module_type', 'module_id']); // Remove session data
...
if (session()->has('pending_comment')) {
    $comment = session()->get('pending_comment'); // just gets, doesn't remove
    ...
    if ($post) {
        ...
        return redirect()->route('account.login')->with('error', __('auth.incorrect_email_password'));
    }
    ...
    public function showUploadCardForm(Request $request) {
        ...
        if (!session()->has('temp_user_id')) {
            return redirect()->route('account.login')->withErrors([__('auth.login_first')]);
        }
        ...
        if (!$user) {
            ...
            return redirect()->route('account.login')->withErrors([__('auth.user_not_found')]);
        }
    ...
    public function verifyStudentCard(Request $req)
    {
        ...
        if (!$user) {
            return redirect()->route('account.register')->withErrors(['auth.user_not_found']);
        }
